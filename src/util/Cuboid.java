...
public class Cuboid {
    ...
    public Cuboid(Location l1, Location l2) throws Exception {
        if (l1 == null || l2 == null) throw new NullPointerException("Location can't be null");
        else if (l1.getWorld() == null) throw new IllegalStateException("Can't create a Cuboid for an unloaded world");
        else if (!l1.getWorld().getName().equals(l2.getWorld().getName())) throw new IllegalStateException("Can't create a Cuboid between two different worlds");
    ...
    public boolean isInside(Location loc) {
        ...
        int y = loc.getBlockY();
        int z = loc.getBlockZ();
        if (x >= x1 && x <= x2) if (y >= y1 && y <= y2) if (z >= z1 && z <= z2) return true;
        return false;
    }
