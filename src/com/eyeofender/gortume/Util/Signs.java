    ...
    public void updateSigns() {
        ...
        for (Sign s : signs) {
            if (s.getLocation().getBlock().getTypeId() == 63 || s.getLocation().getBlock().getTypeId() == 68) {
                 ...
                 * 
                 * if(gm.isInLobby()){ if(gm.getArenaPlayers().size() <
                 * plugin.getConfigHelper().getMaxPlayers()){ s.setLine(0,
                 * ChatColor.GREEN + "[Join]"); }else{ s.setLine(0,
                 * ChatColor.RED + "[Full]"); } }else{ s.setLine(0,
                 * ChatColor.RED + "[In-Game]"); }
                 * 
                 * s.setLine(1, gm.getArenaName()); s.setLine(2, ChatColor.GRAY
                 * + "" + ChatColor.BOLD + gm.getArenaPlayers().size() + "/" +
                 * plugin.getConfigHelper().getMaxPlayers());
                 * 
                 * if(gm.isInLobby()){ s.setLine(3, ChatColor.GOLD +
                 * "-=- Lobby -=-"); }else{ s.setLine(3, ChatColor.GRAY +
                 * "-=- In-Game -=-"); }
                 **/
